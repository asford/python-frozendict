...
static uint64_t _pydict_global_version = 0;
...
static Py_ssize_t
unicodekeys_lookup_generic(PyDictKeysObject* dk, PyObject *key, Py_hash_t hash)
{
...
static Py_ssize_t
dictkeys_generic_lookup(PyDictKeysObject* dk, PyObject *key, Py_hash_t hash)
{
...
_d_Py_dict_lookup() is general-purpose, and may return DKIX_ERROR if (and only if) a
comparison raises an exception.
When the key isn't found a DKIX_EMPTY is returned.
*/
static Py_ssize_t
_d_Py_dict_lookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject **value_addr)
{
    PyDictKeysObject *dk;
    DictKeysKind kind;
    Py_ssize_t ix;
    ...
    if (kind != DICT_KEYS_GENERAL) {
        if (PyUnicode_CheckExact(key)) {
            ...
            ix = unicodekeys_lookup_generic(dk, key, hash);
        }
        ...
        ix = dictkeys_generic_lookup(dk, key, hash);
        if (ix >= 0) {
...
dict_repr(PyDictObject *mp)
{
    ...
    i = 0;
    first = 1;
    while (_d_PyDict_Next((PyObject *)mp, &i, &key, &value)) {
        PyObject *s;
        int res;
...
dict_subscript(PyDictObject *mp, PyObject *key)
{
    ...
    ix = _d_Py_dict_lookup(mp, key, hash, &value);
    if (ix == DKIX_ERROR)
        return NULL;
...
/*[clinic end generated code: output=bba707729dee05bf input=279ddb5790b6b107]*/
{
    ...
    ix = _d_Py_dict_lookup(self, key, hash, &val);
    if (ix == DKIX_ERROR)
        return NULL;
